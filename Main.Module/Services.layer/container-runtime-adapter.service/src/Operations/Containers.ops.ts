...
/*
    Os recursos que `UpdateContainerResources` aceita alterar em um container
    JÁ EM EXECUÇÃO. É um recorte do `resources` do spec — o que a API do Docker
    deixa mudar sem recriar — mais a política de reinício, que também é
    alterável a quente.
*/
type ContainerResourcesUpdate = {
    memoryBytes?: any
    ...
    restart?: { policy?: string, maximumRetryCount?: number | string }
}
...
*/
const ChaveSeDefinida = (chave: string, valor: unknown): Record<string, any> =>
...
*/
const NormalizeExecEnvironment = (env: unknown): string[] => {
    if (Array.isArray(env)) return env.map(String)
    return NormalizeContainerEnvironment(env)
}

const CreateContainerOperations = ({ docker, StreamToBuffer, SafeFileName }: ContainerOperationsContext) => {
    ...
    */
    ...
    /*
        ...
        tentativa.
    */
    const CreateNewContainer = async (spec?: ContainerSpecInput) => {
        ...
        if (!valid) {
            ...
            */
            ...
            const erro: OperationError = new Error(
                unico
                    ? unico.message
                    : `Configuração inválida em: ${errors.map((e: ContainerSpecValidationError) => e.field).join(", ")}.`
            )
    ...
    */
    const GetContainerSpec = async (containerIdOrName: string) => {
    ...
    */
    const AmbienteDaImagem = async (inspecaoDoContainer: DockerPayload): Promise<string[]> => {
    ...
    */
    const RecreateContainer = async ({
        ...
        try {
            ...
            const falha: OperationError = new Error(
    ...
    const RestaurarAntigo = async ({ antigo, nomeOriginal, renomeado, novoId, estavaRodando }: {
        antigo: DockerPayload
        nomeOriginal: string
        renomeado: boolean
        novoId: string | null
        estavaRodando: boolean
    }) => {
    ...
    const RemoveContainer = async (containerIdOrName: string) => {
        try {
    ...
    const StartContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    const StopContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    const RestartContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    const KillContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    /*
        ...
        estado.
    */
    const PauseContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    const UnpauseContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    */
    const RenameContainer = async ({ containerIdOrName, name }: {
        ...
        if (typeof name !== "string" || name.trim() === "") {
            const erro: OperationError = new Error("Informe o novo nome do container.")
    ...
    const InspectContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    */
    const StreamContainerLogs = async ({
        ...
        containerIdOrName: string
        tail?: number | string
        onData?: (evento: { stream: string, data: string }) => void
        ...
        saida.on("data", (pedaco: Buffer) => onData && onData({ stream: "stdout", data: pedaco.toString("utf-8") }))
        erro.on("data", (pedaco: Buffer) => onData && onData({ stream: "stderr", data: pedaco.toString("utf-8") }))

        try {
            container.modem.demuxStream(stream, saida, erro)
        } catch (error) {
            // TTY: o fluxo já vem limpo, sem quadros para separar.
            stream.on("data", (pedaco: Buffer) => onData && onData({ stream: "stdout", data: pedaco.toString("utf-8") }))
        }

        stream.on("error", (error: any) => onError && onError(error))
    ...
    */
    const StreamContainerStats = async ({ containerIdOrName, onData, onError, onEnd }: {
        containerIdOrName: string
        onData?: (amostra: NormalizedContainerStats) => void
        onError?: (erro: any) => void
        onEnd?: () => void
    }) => {
        const container = docker.getContainer(containerIdOrName)
    ...
    const OpenExecSession = async ({
        ...
        */
        const decodificador = CreateDockerStreamDecoder({
            onData: ({ data }: { stream: string, data: string }) => onData && onData(data)
        })

        stream.on("data", (pedaco: Buffer) => decodificador.Push(pedaco))
        stream.on("error", (error: any) => onError && onError(error))
        stream.on("end", () => {
            decodificador.Flush()
            onEnd && onEnd()
        ...
        return {
            Write: (dados: string | Buffer) => {
                try { stream.write(dados) } catch (error) { onError && onError(error) }
            },
            Resize: async ({ cols: colunas, rows: linhas }: { cols?: number, rows?: number }) => {
    ...
    */
    const CreateAndStartContainer = async (spec?: ContainerSpecInput) => {
    ...
    const UltimasLinhasDoLog = async (containerId: string) => {
        try {
    ...
    */
    const UpdateContainerResources = async ({ containerIdOrName, resources = {} }: {
        containerIdOrName: string
        resources?: ContainerResourcesUpdate
    }) => {
        const {
            memoryBytes,
        ...
        if (Object.keys(atualizacao).length === 0) {
            const erro: OperationError = new Error("Informe ao menos um recurso a atualizar.")
    ...
    */
    const TIPOS_DE_MUDANCA: Record<number, string> = { 0: "modified", 1: "added", 2: "deleted" }

    const GetContainerFileSystemChanges = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
            const mudancas = await container.changes()
            return (mudancas || []).map((mudanca: DockerPayload) => ({
                path: mudanca.Path,
    ...
    */
    const GetContainerStatsSnapshot = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    */
    const PruneContainers = async ({ filters }: { filters?: DockerFilters } = {}) => {
        try {
    ...
    */
    const RunExec = async ({
        ...
        const decodificador = CreateDockerStreamDecoder({
            onData: ({ stream: fluxo, data }: { stream: string, data: string }) => {
        ...
        const timedOut = await new Promise<boolean>((resolve, reject) => {
            ...
            const Encerrar = (expirou: boolean) => {
                if (encerrado) return
                encerrado = true
    ...
    const GetContainerLogHistory = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
    ...
    const ExportContainer = async (containerIdOrName: string) => {
        try {
            const container = docker.getContainer(containerIdOrName)
