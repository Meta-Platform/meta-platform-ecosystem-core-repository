...
const NormalizedLabels = (labels: Record<string, unknown>) => {
    return Object.fromEntries(
        Object.entries(labels).map(([key, value]) => [ key, value == null ? "" : String(value) ])
    )
}

const CreateVolumeOperations = ({ docker, SafeFileName, ephemeral }: VolumeOperationsContext) => {
    ...
    */
    const ListAllVolumes = async ({ filters }: { filters?: DockerFilters } = {}) => {
        try {
    ...
    const InspectVolume = async (volumeName: string) => {
        try {
    ...
    */
    const CreateNewVolume = async (options: DockerPayload) => {
    ...
    */
    const RemoveVolume = async (volumeNameOrOptions: string | { volumeName?: string, force?: boolean }) => {
        const { volumeName, force } = typeof volumeNameOrOptions === "string"
            ? { volumeName: volumeNameOrOptions, force: false }
            : (volumeNameOrOptions ?? {})

        if (!volumeName) {
            const erro: OperationError = new Error("Informe o nome do volume a remover.")
    ...
    */
    const PruneVolumes = async ({ filters }: { filters?: DockerFilters } = {}) => {
        try {
    ...
    */
    const ContainersUsingVolume = async (volumeName: string) => {
        const containers = await docker.listContainers({ all: true })

        return containers
            .filter((container: DockerPayload) =>
                (container.Mounts || []).some((montagem: DockerPayload) => montagem.Name === volumeName))
            .map((container: DockerPayload) => ({
                id: container.Id,
    ...
    */
    const ExigirVolumeLivre = async (volumeName: string, force: boolean | undefined, operacao: string) => {
        if (force) return []

        const usuarios = await ContainersUsingVolume(volumeName)
        const rodando = usuarios.filter((usuario: DockerPayload) => usuario.running)
        if (rodando.length === 0) return usuarios

        const erro: OperationError = new Error(
            `Não é possível ${operacao} o volume ${volumeName}: ` +
            `${rodando.map((u: DockerPayload) => u.name).join(", ")} ${rodando.length === 1 ? "está" : "estão"} usando. ` +
    ...
    const GetVolumeUsage = async (volumeName: string) => {
    ...
    */
    const ImportVolume = async ({ volumeName, contentBase64, clear = false, force = false }: {
        ...
        if (!volumeName || !contentBase64) {
            const erro: OperationError = new Error("Informe o volume de destino e o conteúdo do backup.")
    ...
    const EsvaziarConteudo = async (volumeName: string) => {
        let container: DockerHandle
        try {
            container = await ephemeral.CreateEphemeralVolumeContainer({
                volumeName,
    ...
    const EmptyVolume = async ({ volumeName, force = false }: { volumeName: string, force?: boolean }) => {
    ...
    */
    const CloneVolume = async ({ sourceVolumeName, targetVolumeName, force = false }: {
        ...
        if (!sourceVolumeName || !targetVolumeName) {
            const erro: OperationError = new Error("Informe o volume de origem e o de destino.")
