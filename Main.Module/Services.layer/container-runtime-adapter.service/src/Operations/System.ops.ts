...
import type { DockerFilters, DockerPayload, OperationError, SystemOperationsContext } from "./Types"
...
const CreateSystemOperations = ({ docker }: SystemOperationsContext) => {
    ...
    let temporizadorDeReconexao: ReturnType<typeof setTimeout> | null = null
    let desligado = false
    ...
    */
    const RegisterDockerEventListener = (f: (eventData: DockerPayload) => void) => {
        const ouvinte = (eventData: DockerPayload) => f(eventData)
    ...
    */
    const StreamRuntimeEvents = ({ filters, since, onData, onError, onEnd }: {
        ...
        since?: number | string,
        onData?: (evento: DockerPayload) => void,
        onError?: (erro: unknown) => void,
        onEnd?: () => void
    } = {}) => {
        let fechado = false

        const Normalizar = (evento: DockerPayload) => ({
        ...
        */
        const tipos: any[] | null = filters?.type ? [].concat(filters.type) : null
        const acoes: any[] | null = filters?.action ? [].concat(filters.action) : null

        const Interessa = (evento: ReturnType<typeof Normalizar>) =>
    ...
    */
    const GetDiskUsage = async () => {
        try {
            ...
            const Somar = (lista: any[], Valor: (item: any) => number | undefined) =>
                lista.reduce((total, item) => total + (Valor(item) || 0), 0)
            ...
            const recuperavel: {
                ...
                images: Somar(imagens.filter((i: any) => (i.Containers ?? 0) === 0), (i: any) => i.Size),
                containers: Somar(containers.filter((c: any) => c.State !== "running"), (c: any) => c.SizeRw),
                volumes: Somar(volumes.filter((v: any) => (v.UsageData?.RefCount ?? 0) === 0), (v: any) => v.UsageData?.Size),
                buildCache: Somar(cacheDeBuild.filter((c: any) => !c.InUse), (c: any) => c.Size)
            }
            ...
            return {
                ...
                totals: {
                    images: Somar(imagens, (i: any) => i.Size),
                    containers: Somar(containers, (c: any) => c.SizeRw),
                    volumes: Somar(volumes, (v: any) => v.UsageData?.Size),
                    buildCache: Somar(cacheDeBuild, (c: any) => c.Size)
                },
    ...
    */
    const PruneSystem = async ({ types, danglingOnly = true, filters }: {
        ...
        const PODAS: Record<string, () => Promise<{ deleted: any[], spaceReclaimed: number }>> = {
            containers: async () => {
        ...
        const perType: Record<string, { deleted: any[], spaceReclaimed: number }> = {}
        const errors: { type: string, code: string | null, message: string }[] = []
