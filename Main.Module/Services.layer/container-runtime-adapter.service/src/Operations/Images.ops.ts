...
const CreateImageOperations = ({ docker, StreamToBuffer, SafeFileName }: ImageOperationsContext) => {

    /*
        Chamada sem argumento mantém o comportamento de antes (CTMG-41).
        `filters` aceita reference, label, dangling, before, since.
    */
    const ListAllImages = async ({ all, filters }: { all?: boolean, filters?: DockerFilters } = {}) => {
        try {
    ...
    const InspectImage = async (imageIdOrName: string) => {
        try {
    ...
    const RemoveImage = async ({ imageIdOrName, force = false }: { imageIdOrName: string, force?: boolean }) => {
        try {
            const image = docker.getImage(imageIdOrName)
    ...
    const ExportImage = async (imageIdOrName: string) => {
        try {
            const image = docker.getImage(imageIdOrName)
    ...
    */
    const PullImage = async ({ reference, platform, auth, onProgress }: {
        ...
        platform?: string
        auth?: DockerPayload
        onProgress?: (progresso: DockerPayload) => void
    }) => {
        if (typeof reference !== "string" || reference.trim() === "") {
            const erro: OperationError = new Error("Informe a referência da imagem, ex.: postgres:16.")
        ...
        await new Promise<void>((resolve, reject) => {
            ...
            docker.pull(referencia, opcoes, (erro: any, stream: any) => {
                if (erro) {
                    /*
                ...
                docker.modem.followProgress(
                    ...
                    (falha: any, saida: DockerPayload[]) => {
                        ...
                        */
                        ...
                        if (evento) {
                            const falhaDeCamada: OperationError = new Error(
    ...
    */
                ...
                (erro: any, stream: any) => {
                    ...
                    docker.modem.followProgress(
                        ...
                        (falha: any, saida: DockerPayload[]) => {
                            ...
                            const evento = (saida || []).find((linha) => linha && linha.error)
                            if (evento) {
                                const falhaDeEnvio: OperationError = new Error(
    ...
    const TagImage = async ({ imageIdOrName, repo, tag }: {
        ...
        if (!repo) {
            const erro: OperationError = new Error("Informe o repositório de destino, ex.: meu/app.")
    ...
    */
    const GetImageHistory = async (imageIdOrName: string) => {
        try {
            const historico = await docker.getImage(imageIdOrName).history()
            return (historico || []).map((camada: DockerPayload) => ({
                id: camada.Id,
    ...
    const SearchImages = async ({ term, limit, filters }: {
        ...
        if (!term) {
            const erro: OperationError = new Error("Informe o termo da busca.")
        ...
        try {
            ...
            return (resultados || []).map((item: DockerPayload) => ({
                name: item.name,
    ...
    */
    const LoadImage = async ({ contentBase64, onProgress }: {
        ...
        onProgress?: (evento: DockerPayload) => void
    }) => {
        if (!contentBase64) {
            const erro: OperationError = new Error("Informe o conteúdo do arquivo tar da imagem.")
        ...
        const saida = await new Promise<DockerPayload[]>((resolve, reject) => {
            docker.loadImage(fluxo, {}, (erro: any, stream: any) => {
                if (erro) return reject(erro)

                docker.modem.followProgress(
                    stream,
                    (falha: any, resultado: DockerPayload[]) => falha ? reject(falha) : resolve(resultado || []),
                    (evento: DockerPayload) => typeof onProgress === "function" && onProgress(evento)
                )
            })
        })
    ...
    const PruneImages = async ({ dangling = true, filters }: { dangling?: boolean, filters?: DockerFilters } = {}) => {
        /*
    ...
    */
    const BuildImageFromDockerfileString = async ({
        ...
        buildargs?: DockerPayload
        contextTarStream?: any
        imageTagName?: string
        onData?: (chunk: Buffer) => void
    }) => {
        return new Promise((resolve, reject) => {

            docker.buildImage(contextTarStream, { t: imageTagName, buildargs }, (err: any, stream: any) => {
                ...
                stream.on('end', async () => {
                    if (buildError) {
                        const error: OperationError = new Error(`Falha no build da imagem ${imageTagName}: ${buildError}`)
    ...
    */
    const BuildImageFromDockerfileContent = async ({
        ...
        imageTagName?: string
        dockerfileContent?: unknown
        buildargs?: DockerPayload
        onData?: (chunk: Buffer) => void
    }) => {
        if (typeof dockerfileContent !== "string" || dockerfileContent.trim() === "") {
            const erro: OperationError = new Error("O conteúdo do Dockerfile é obrigatório.")
