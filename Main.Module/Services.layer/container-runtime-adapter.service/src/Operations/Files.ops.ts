...
/** O mínimo que `PorPastaDepoisNome` precisa saber de uma entrada. */
type EntradaOrdenavel = { name: string, isDirectory: boolean }
...
const {
...
} = require("../Helpers/TarSingleFile") as {
    BuildTarWithSingleFile: (options: {
        name?: unknown
    ...
    /*
        `content` NÃO é opcional aqui: ao contrário da listagem, esta função
        lê o tar pedindo o conteúdo, e só devolve entrada que não é diretório.
        Ou vem o arquivo inteiro, ou vem `null`.
    */
    ExtractFirstFileFromTar: (buffer: Buffer) => { name: string, content: Buffer, size: number } | null
    ReadTarEntriesFromStream: (stream: any) => Promise<{ entries: TarEntry[], truncated: boolean }>
}
/*
...
*/
const CreateFileOperations = ({ docker, StreamToBuffer, ephemeral, RunExec }: FileOperationsContext) => {
    ...
    const RequireSafePath = (relativePath: unknown) => RequireSafeVolumePath(relativePath)
    ...
    */
    const ListVolumeEntries = async ({ volumeName, path }: { volumeName: string, path?: string }) => {
        ...
        try {
            ...
            if (resultado.statusCode === 3) {
                const error: OperationError = new Error("Caminho não encontrado dentro do espaço.")
            ...
            const entries = resultado.stdout
                ...
                .filter((entry): entry is DirectoryEntry => entry !== null && entry.name.length > 0)
    ...
    */
    ...
    const GetFileFromVolume = async ({ volumeName, path }: { volumeName: string, path?: string }) => {
        const alvo = RequireSafePath(path)
        if (alvo.relative === "") {
            const error: OperationError = new Error("Informe o arquivo a baixar.")
        ...
        try {
            ...
            try {
                ...
                const error: OperationError = new Error("Arquivo não encontrado dentro do espaço.")
                error.code = "PATH_NOT_FOUND"
            ...
            if (!arquivo) {
                const error: OperationError = new Error("O caminho informado não é um arquivo.")
    ...
    const DeleteVolumeEntry = async ({ volumeName, path }: { volumeName: string, path?: string }) => {
        const alvo = RequireSafePath(path)
        if (alvo.relative === "") {
            // Apagar a raiz seria esvaziar o espaço inteiro por um caminho
            // vazio — remoção de espaço tem caminho próprio, com confirmação.
            const error: OperationError = new Error("Não é possível remover a raiz do espaço por aqui.")
            error.code = "INVALID_PATH"
            throw error
        }
    ...
    const ContainerEstaRodando = async (containerIdOrName: string) => {
    ...
    */
    const ExigirCaminhoDeContainer = (path: unknown): string => {
        if (typeof path !== "string" || path.trim() === "") {
            const erro: OperationError = new Error("Informe o caminho dentro do container.")
    ...
    const ListContainerEntries = async ({ containerIdOrName, path = "/" }: { containerIdOrName: string, path?: string }) => {
    ...
    const ListarComExec = async (containerIdOrName: string, alvo: string) => {
        ...
        if (resultado.exitCode === 3) {
            const erro: OperationError = new Error(`Caminho não encontrado no container: ${alvo}`)
        ...
        */
        ...
        const entries = resultado.stdout
            ...
            .filter((entrada): entrada is DirectoryEntry => entrada !== null && entrada.name.length > 0)
            .sort(PorPastaDepoisNome)

        return { path: alvo, entries, source: "exec" }
    }

    const ListarComArquivo = async (containerIdOrName: string, alvo: string) => {
        let stream
        try {
    ...
    /* Serve às duas listagens — a do exec e a dos cabeçalhos do tar. */
    const PorPastaDepoisNome = (a: EntradaOrdenavel, b: EntradaOrdenavel) => (a.isDirectory === b.isDirectory)
        ? a.name.localeCompare(b.name)
        : (a.isDirectory ? -1 : 1)

    const CopyFromContainer = async ({ containerIdOrName, path }: { containerIdOrName: string, path?: unknown }) => {
        const alvo = ExigirCaminhoDeContainer(path)
        ...
        try {
            ...
            const erro: OperationError = new Error(`Arquivo não encontrado no container: ${alvo}`)
            erro.code = "PATH_NOT_FOUND"
            erro.httpStatus = 404
            erro.statusCode = 404
            erro.cause = error
        ...
        if (!arquivo) {
            const erro: OperationError = new Error("O caminho informado não é um arquivo.")
    ...
    */
    const ExigirContainerRodando = async (containerIdOrName: string, operacao: string) => {
    ...
    const DeleteContainerEntry = async ({ containerIdOrName, path }: { containerIdOrName: string, path?: unknown }) => {
        const alvo = ExigirCaminhoDeContainer(path)

        if (alvo === "/") {
            const erro: OperationError = new Error("Não é possível remover a raiz do container.")
            erro.code = "INVALID_PATH"
            erro.httpStatus = 400
            erro.statusCode = 400
            throw erro
    ...
    const MakeContainerDirectory = async ({ containerIdOrName, path }: { containerIdOrName: string, path?: unknown }) => {
        const alvo = ExigirCaminhoDeContainer(path)
    ...
    */
    const MakeVolumeDirectory = async ({ volumeName, path }: { volumeName: string, path?: string }) => {
        const alvo = RequireSafePath(path)
    ...
    */
    const GetFileChunkFromVolume = async ({ volumeName, path, offsetMiB, lengthMiB }: {
        volumeName: string
        path?: string
        offsetMiB?: number | string
        lengthMiB?: number | string
    }) => {
        const alvo = RequireSafePath(path)
        if (alvo.relative === "") {
            const error: OperationError = new Error("Informe o arquivo a baixar.")
        ...
        if (!Number.isInteger(inicio) || inicio < 0 || !Number.isInteger(tamanho) || tamanho <= 0) {
            const error: OperationError = new Error("offsetMiB e lengthMiB precisam ser inteiros (em MiB).")
    ...
    */
    const PutFileChunkInVolume = async ({ volumeName, path, fileName, contentBase64, partIndex, isLast }: {
        ...
        if (!Number.isInteger(parteNumero) || parteNumero < 0) {
            const error: OperationError = new Error("partIndex precisa ser um inteiro não negativo.")
    ...
    */
    const InspectVolumeUpload = async ({ volumeName, path, fileName }: { volumeName: string, path?: string, fileName?: unknown }) => {
        const destino = RequireSafePath(path)
        const nome = RequireSafeFileName(fileName)
    ...
    */
    const MoveVolumeEntry = async ({ volumeName, path, destinationPath }: {
        ...
        if (origem.relative === "") {
            const error: OperationError = new Error("Não é possível mover a raiz do espaço.")
            error.code = "INVALID_PATH"
            throw error
        }
        if (destino.relative === "") {
            const error: OperationError = new Error("Informe o destino.")
            error.code = "INVALID_PATH"
            throw error
        }
        ...
        */
        if (destino.relative.indexOf(`${origem.relative}/`) === 0) {
            const error: OperationError = new Error("Não é possível mover uma pasta para dentro dela mesma.")
            error.code = "INVALID_PATH"
            throw error
        }
        ...
        try {
            ...
            if (resultado.statusCode === 3) {
                const error: OperationError = new Error("O item que se quer mover não existe mais.")
                error.code = "PATH_NOT_FOUND"
                throw error
            }
            if (resultado.statusCode === 4) {
                const error: OperationError = new Error("Já existe um item com esse nome no destino.")
