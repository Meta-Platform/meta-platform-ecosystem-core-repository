...
const Erro = (field: string, code: string, message: string): ContainerSpecValidationError => ({ field, code, message })

const ValidateContainerSpec = (spec: ContainerSpec = {}): ContainerSpecValidation => {
    const errors: ContainerSpecValidationError[] = []
...
*/
const BuildHealthcheck = (healthcheck?: ContainerHealthcheckSpec) => {
    ...
    const EmNanos = (segundos: unknown) => segundos === undefined || segundos === null || segundos === ""
        ? undefined
...
const BuildRestartPolicy = (restart?: ContainerRestartSpec) => {
...
const BuildResources = (resources: ContainerResourcesSpec = {}) => {
    const ComValor = (chave: string, valor: unknown): Record<string, any> =>
        valor === undefined || valor === null || valor === "" ? {} : { [chave]: valor }

    const Bytes = (valor: unknown) => Number(valor) === -1 ? -1 : ParseByteSize(valor)
...
const NormalizeStringList = (valor: unknown): string[] | undefined => {
...
const NormalizarMapaDeTexto = (mapa?: Record<string, unknown> | null) =>
