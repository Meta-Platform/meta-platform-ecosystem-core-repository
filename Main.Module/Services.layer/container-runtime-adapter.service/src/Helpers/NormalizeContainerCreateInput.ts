...
*/
const CriarErro = (code: string, message: string, field?: string): OperationError => {
    const erro: OperationError = new Error(message)
    erro.code = code
    erro.httpStatus = 400
    erro.statusCode = 400
...
*/
const NormalizeMount = (montagem: any, indice: number) => {
...
const NormalizeMounts = (mounts: unknown = []) => {
