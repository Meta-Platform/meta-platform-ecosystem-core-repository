...
/*
    ...
    faz.
*/
import type { DockerPayload, NormalizedContainerStats } from "../Operations/Types"

const MemoriaUsada = (amostra: DockerPayload) =>
    (amostra.memory_stats?.usage || 0) - (amostra.memory_stats?.stats?.cache || 0)

const PercentualDeCpu = (amostra: DockerPayload): number => {
...
const TotalDeRede = (amostra: DockerPayload) => {
...
const TotalDeBloco = (amostra: DockerPayload) =>
    (amostra.blkio_stats?.io_service_bytes_recursive || [])
        .reduce((total: { read: number, write: number }, entrada: DockerPayload) => {
...
const NormalizeContainerStats = (amostra: DockerPayload): NormalizedContainerStats => {
