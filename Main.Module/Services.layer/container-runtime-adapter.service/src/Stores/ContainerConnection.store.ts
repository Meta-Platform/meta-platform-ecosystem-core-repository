...
import type { ContainerConnection } from "../Managers/Types"

const { readFileSync, writeFileSync, renameSync, mkdirSync, existsSync } = require("node:fs") as typeof import("node:fs")
const { dirname, join } = require("node:path") as typeof import("node:path")

const NOME_DO_ARQUIVO = "container-connections.json"
const SCHEMA_VERSION = 1

type ContainerConnectionStoreParams = {
    filePath?: string
    storageDir?: string
}

const ContainerConnectionStore = (params: ContainerConnectionStoreParams = {}) => {
    ...
    // A guarda acima já garantiu que um dos dois veio; o verificador não a segue.
    const caminho = filePath || join(storageDir as string, NOME_DO_ARQUIVO)
    ...
    */
    const Load = (): ContainerConnection[] => {
    ...
    const Save = (connections: ContainerConnection[]): ContainerConnection[] => {
        GarantirDiretorio()
        const documento = {
