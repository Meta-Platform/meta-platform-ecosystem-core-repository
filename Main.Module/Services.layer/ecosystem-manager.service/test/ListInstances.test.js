...
if (!globalThis.Log) {
    const nada = () => {}
    globalThis.Log = { info : nada, error : nada, debug : nada, warn : nada, fatal : nada, trace : nada, message : nada }
}

const CreateLibDouble = (implementacoes) => ({
    require : (nome) => {
...
const CreateManager = ({ applicationTasks = [], executions = {} }) => {
    ...
    const environmentRuntimeService = {
        ...
        GetExecutionData           : (executionId) => executions[String(executionId)],
    ...
    let instanceStoreCapturado = null
    ...
    const manager = EcosystemManager({
        ...
        instanceStoreLib           : CreateLibDouble({
            "InitializeInstanceStore" : (filePath) => {
