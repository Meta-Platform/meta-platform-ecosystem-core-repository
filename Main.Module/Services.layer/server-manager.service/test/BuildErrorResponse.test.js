...
test("erro de contrato preserva status, code e mensagem", () => {
    const erro = new Error("O adaptador de runtime de containers não está respondendo no socket.")
...
test("statusCode sozinho basta — nem todo erro do ecossistema traz httpStatus", () => {
    const erro = new Error("Sem usuário autenticado não há em nome de quem agir.")
...
test("status fora da faixa de erro HTTP não é obedecido", () => {
    const erro = new Error("código de erro de socket, não de HTTP")
