...
const CreateFakeWebSocket = () => {
    ...
    return {
        ...
        on         : function (event: string, handler: any) { handlers[event] = handler },
        close      : function (code: number, reason?: string) { this.closedWith = { code, reason } },
        Emit       : (event: string) => handlers[event] && handlers[event]()
    }
}

const CreateRequest = (token: string) => ({ cookies : { token }, headers : {} })

const OpenSocket = async ({ token, tokenRevocationCheckerService, revocationRecheckIntervalMs }: {
    token                          : string,
    tokenRevocationCheckerService ?: any,
    ...
    let erroDaAbertura: any = null
    await middlewareService.GetWebSocketMiddleware()(ws, CreateRequest(token), (error: any) => { erroDaAbertura = error })
