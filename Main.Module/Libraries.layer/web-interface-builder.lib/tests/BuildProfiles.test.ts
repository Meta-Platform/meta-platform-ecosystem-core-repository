...
describe("BuildProfiles — resolução", () => {
    ...
    it("nome desconhecido não derruba nada: volta ao padrão", () => {
        ...
        globalThis.Log = { warn: (_: any, message: any) => warnings.push(message) }
...
describe("BuildProfiles — assinatura para o cache", () => {
    ...
    it("perfis diferentes têm assinaturas diferentes", () => {
        const keys = [RELEASE, DEBUG, DEBUG_WATCH]
            .map((name: string) => GetProfileFingerprintKey(ResolveBuildProfile({ profileName: name, env: {} })))
...
const FAKE_WEBPACK = Object.assign(() => {}, {
    DefinePlugin:   class { constructor(d: any){ (this as any).d = d } },
    ProgressPlugin: class { constructor(h: any){ (this as any).h = h } }
})
const FakeHtmlWebpackPlugin = class { constructor(o: any){ (this as any).o = o } }
...
const _Config = (profileName: string, extra: Record<string, any> = {}) => CreateWebpackConfig({
...
const _Rule = (config: any, test: RegExp) => config.module.rules.find((rule: any) => String(rule.test) === String(test))
...
describe("CreateWebpackConfig — o perfil chega na configuração", () => {
    ...
    it("o ProgressPlugin só entra quando há barra de progresso esperando", () => {
        ...
        assert.equal(semBarra.plugins.some((p: any) => p instanceof FAKE_WEBPACK.ProgressPlugin), false)

        const comBarra = _Config(RELEASE, { onProgress: () => {} })
        assert.equal(comBarra.plugins.some((p: any) => p instanceof FAKE_WEBPACK.ProgressPlugin), true)
    })
    ...
    it("preserva o que já funcionava: asset modules, aliases e resolução", () => {
        ...
        const assetRule = config.module.rules.find((rule: any) => rule.type === "asset/resource")
        assert.ok(assetRule)
