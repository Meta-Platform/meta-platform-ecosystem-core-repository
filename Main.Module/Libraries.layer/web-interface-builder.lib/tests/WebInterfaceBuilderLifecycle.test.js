...
const CreateFakeWebpack = ({ runOutcome = "success", watchOutcome = "success" } = {}) => {

    const calls = { compilerCount: 0, runCount: 0, watchCount: 0, closeCount: 0, watchingCloseCount: 0 }

    const _MakeStats = ({ errors = [] } = {}) => ({
        compilation: {
            errors,
            warnings: [],
    ...
    const webpack = () => {
        calls.compilerCount++
        return {
            run: (callback) => { calls.runCount++; _Emit(runOutcome, callback) },
            watch: (options, callback) => {
                calls.watchCount++
                assert.ok(options, "watch precisa receber watchOptions")
                _Emit(watchOutcome, callback)
                return { close: (done) => { calls.watchingCloseCount++; done && done() } }
            },
            close: (done) => { calls.closeCount++; done && done() }
        }
    }

    webpack.DefinePlugin   = class { constructor(definitions){ this.definitions = definitions } }
    webpack.ProgressPlugin = class { constructor(handler){ this.handler = handler } }

    return { webpack, calls }
}

const CreateBuilderWith = (fake, { requireLog } = {}) => {
    const FakeHtmlWebpackPlugin = class { constructor(options){ this.options = options } }
