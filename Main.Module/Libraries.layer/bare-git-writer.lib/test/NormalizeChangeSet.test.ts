...
const Put = (path: string, content: string = "conteudo"): any => ({ op: "put", path, content })

const AssertRefused = (changes: any[], code: string): void => {
    assert.throws(() => NormalizeChangeSet(changes), (error: any) => {
...
test("aceita caminho com vírgula, acento e espaço", () => {
    ...
    const normalized = NormalizeChangeSet(paths.map((path: string) => Put(path)))
    assert.deepStrictEqual(normalized.map(({ path }: any) => path), paths)
})
...
test("aplica os tetos de tamanho e de quantidade", () => {
    ...
    AssertRefused([Put("a.js")].concat(new Array(DEFAULT_LIMITS.maxChanges).fill(0).map((_: any, i: number) => Put(`f${i}.js`))), "TOO_MANY_CHANGES")
