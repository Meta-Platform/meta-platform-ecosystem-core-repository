...
const WithRepository = async (Body: (context: any) => Promise<void>): Promise<void> => {
...
const Commit = (writer: any, gitDirPath: string, changes: any[], extra: any = {}): Promise<any> => writer.WriteCommit({
...
test("branches: cria, lista e apaga com compare-and-swap", async () => {
    await WithRepository(async ({ gitDirPath, writer }) => {
        ...
        assert.deepStrictEqual((await writer.ListBranches({ gitDirPath })).map(({ name }: any) => name).sort(),
            ["main", "trabalho"])
        ...
        assert.deepStrictEqual((await writer.ListBranches({ gitDirPath })).map(({ name }: any) => name), ["main"])
    })
})
...
test("rascunho vive FORA da história: nem branch, nem tag, nem no log", async () => {
    await WithRepository(async ({ gitDirPath, writer }) => {
        ...
        */
        assert.deepEqual((await writer.ListBranches({ gitDirPath })).map(({ name }: any) => name), ["main"],
